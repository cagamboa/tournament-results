Ready file for FSND Project #2

This folder contains 4 files:

ReadMe — a text file of instructions

tournament.sql - a file of the sql commands used to generate the 		 tables and views to recreate the tournament database.
...
1. connect to the database tournament. The tournament.sql file has the create db and connect to db commands in it. If you have already executed them, these two commands will generate a warning. 
At the prompt type:    \i tournament.sql

The database should now have two tables: tournament and matches
It should also have 5 views: wins, losses, totalMatches, totals, standings.

2. In a python window type in python tournament_test.py
3. The output of this file is a listing of all the test results. There are 8 tests, all of them should pass. The final message for a successful test is: 
